...
def Find_term(term_name):
    ...
    print('\nNumber of Guitar types by roughly computing: %d') % Corpus.getcol(vectorizer.vocabulary_.get(term_name)).sum()
    ...
    new_documents = [ line for line in open(txtfile) if term_name in line.lower().split()]
    texts = [[word for word in document.lower().split() if word not in stop_list]
             for document in new_documents]
    
    types = []
    for line in texts:
        types.append(line[line.index(term_name)-1])
